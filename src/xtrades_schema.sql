...
VALUES
    ('traderJoe', 'Joe the Trader', TRUE, 'Focuses on wheel strategy with tech stocks'),
    ('optionsGuru', 'Options Master', TRUE, 'Specializes in credit spreads'),
    ('wheelKing', 'Wheel Strategy King', FALSE, 'Currently inactive - testing historical data')
...
SELECT
    ...
    'AAPL CSP: STO 1x $170 PUT @ $2.50 exp ' || TO_CHAR(CURRENT_DATE + INTERVAL '30 days', 'MM/DD/YYYY'),
    NOW() - INTERVAL '7 days'
FROM xtrades_profiles p
WHERE p.username = 'traderJoe'
ON CONFLICT DO NOTHING;

INSERT INTO xtrades_trades (
    profile_id, ticker, strategy, action, entry_price, entry_date,
    exit_price, exit_date, quantity, pnl, pnl_percent, status,
...
SELECT
    ...
    'TSLA CSP: Closed for 50% profit - BTC @ $2.50',
    NOW() - INTERVAL '10 days'
FROM xtrades_profiles p
WHERE p.username = 'optionsGuru'
ON CONFLICT DO NOTHING;
...
VALUES
    (3, 15, 10, 5, 12.5, 'success'),
    (3, 20, 15, 5, 18.3, 'success'),
    (2, 8, 5, 3, 9.1, 'partial');
